...
def _load_fallback():
    ...
    logfire.info("Loading fallback embeddings (sentence-transformers/all-MiniLM-L6-v2, 768-dim).")
...
def _embed_batch(batch: list[str]) -> list[list[float]]:
    if _model_type == "gemini":
        ...
        for attempt in range(4):
            ...
            return _active_model.encode(batch, show_progress_bar=True).tolist()    
...
def embed_texts(texts: list[str]) -> list[list[float]]:
    ...
    for i in range(0, len(texts), BATCH_SIZE):
        ...
        with logfire.span("Embed batch", model = _model_type, start=i, batch_size=len(batch)):
